...
proc SQliteOpen {} {
  ...
  if {$gaSet(radNet)} {  
    set dbFile \\\\prod-svm1\\tds\\temp\\SQLiteDB\\ilyaJerAteStats.db
...
proc SQliteAddLine {pair} {
  ...
  puts "[MyTime] SQliteAddLine $pair $barcode"
  if {[string match *skip* $barcode]} {
    ## do not include skipped in stats
    return 0
  }
  if {$gaSet($pair.barcode1.IdMacLink)=="link"} {
...
proc AddLine {} {
  ...
  set hostDescription IlyaGinzburgPC
  set status Pass
  set gaSet(pair) wert
