...
proc RegBC {pair} {
  global gaSet gaDBox
  Status "BarCode Registration $pair"
  set ret  -1
  set res1 -1
  set res2 -1
...
#   set pairIndx -1
  ...
  #foreach pair $::pair  {}
    ...
    foreach la {1} {
      ...
      update
      AddToPairLog $pair "MAC:$mac IDbarcode:$barcode"
      #after 1000
      if {[set res$la]!="0"} {
        puts "ret:[set res$la]"
        set ret -1
...
#     AddToLog "mac:$mac Barcode-1 - $barcode1"
    ...
    #foreach pair [PairsToTest]  {}
      foreach la {1} {
        puts $id "[MyTime] Tester:$pair MAC:$gaSet($pair.mac$la) BarCode:$gaSet($pair.barcode$la)"
      }      
...
proc CheckBcOk {lPassPair} {
  ...
  if {$gaSet(useExistBarcode)==0} {
    ...
    if {[string match *.12CMB.* $gaSet(DutInitName)]==1} {
      set tit "Read front panel's Barcode"
    ...
    set ret [DialogBox -title $tit -text $tex -ent1focus 1\
        -type "Ok Cancel" -entQty [llength $lPassPair]  -entPerRow 1 \
        -entLab $entLab -icon /images/uut48.ico]
    ...
    foreach pair $lPassPair {
      if ![info exists gaSet($pair.barcode1)] {
        ...
        set gaDBox(entVal$pair) $gaSet($pair.barcode1)
...
proc ReadBarcode {lPassPair} {
  ...
  while {$ret != "0" } {
    ...
    if { $ret == "-2" ||  $ret == "-1" } {
      foreach pair $lPassPair {
        set gaSet(log.$pair) c:/logs/$pair.${gaSet(logTime)}.txt
        AddToPairLog $pair "$gaSet(DutFullName)"
      }
      return $ret
    }
	}	
  Status ""
  set pairIndx -1
  puts "ReadBarcode gaDBox:<[lsort -dict [array names gaDBox entVal*]]>"
  foreach {ent1} [lsort -dict [array names gaDBox entVal*]] {
    incr pairIndx
    set pair [lindex $lPassPair $pairIndx]
    foreach uut {1} ent {1} {
...
#         set pa $gaSet(pair) 
#       }
      puts "ReadBarcode pair:<$pair> barcode:<$barcode> logTime:<$gaSet(logTime)>"
      set gaSet(log.$pair) c:/logs/$pair.${gaSet(logTime)}-$barcode.txt
      CheckMac $barcode $pair $uut
      AddToPairLog $pair "$gaSet(DutFullName)"
      AddToPairLog $pair "ID Barcode: - $barcode"
      AddToPairLog $pair "UUT: $pair"
