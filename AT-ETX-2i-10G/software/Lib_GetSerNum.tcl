...
proc GetSerNum {args} {
  ...
  while 1 {
    ...
    set ret [GetSnDlg -title $ti -text "$te" -type "Ok Cancel" -icon $icon]     
    #puts "\n<$ret> was clicked\n" 
    if {$ret=="Cancel"} {
      return -1
    }
    set serNum [string toupper $gaGetSnDBox(entVal1)]
    puts "entryValue:<$serNum>"
    if {[string length $serNum]==10  && [string is digit $serNum]} {
      return $serNum      
    } else {
      ## try again
      set te "$serNum is not valid\nTry again"
    }
  }
}
# ***************************************************************************
...
proc GetSnEntryValidCmd {P dotEn dashEn} {
  ...
  #puts "P:<$P> leng:<$leng> rep:<$rep> num:<$num> $dot $dash"
...
proc WritePage0_SN {} {
  ...
  puts "[MyTime] WritePage0_SN"; update
  ...
  set res "$var1 $var2"
  set resL [split $res " "]
  set l1 [list 00 00 00 [string range $gaSet(serialNum) 0 1] [string range $gaSet(serialNum) 2 3]\
   [string range $gaSet(serialNum) 4 5] [string range $gaSet(serialNum) 6 7]  [string range $gaSet(serialNum) 8 9]] 
...
proc GetVneNum {args} {
  ...
  set tiIndx [lsearch $args "-ti"]
  if {$tiIndx=="-1"} {
    set ti "Get VNE Number"
  } else {
    set ti [lindex $args [expr {$tiIndx+1}]]    
  }
  set teIndx [lsearch $args "-te"]
  if {$teIndx=="-1"} {
    set te "UUT's VNE Number "
  } else {
    set te [lindex $args [expr {$teIndx+1}]]    
  }
  ...
  while 1 {
    RLSound::Play information
    set ret [GetSnDlg -title $ti -text "$te" -type "Ok Cancel" -icon $icon -width 40]     
    #puts "\n<$ret> was clicked\n" 
    if {$ret=="Cancel"} {
      return -1
    }
    set entryValue [string trim $gaGetSnDBox(entVal1)]
        
    ## initial set
    foreach {txt vneNum} {TXT VNENUM} {break}
    
    if {[string match {*-*} $entryValue] && [string length $entryValue]>2} {
      ## HTTPS://VORBOSS.COM/Q/VNE-10653293
      ## hfhfdg-384
      foreach {txt vneNum} [split $entryValue -] {break} 
    } elseif {[string is digit $entryValue]} {
      ## 10653293
      set vneNum $entryValue
    }
    puts "\nGetVneNum entryValue:<$entryValue> txt:<$txt> vneNum:<$vneNum>\n"
    if {[string is digit $vneNum] && [string length $vneNum]>0} {
      return $vneNum      
    } else {
      ## try again
      set te "$entryValue is not valid VNE Number\nTry again"
    }
  }
}
