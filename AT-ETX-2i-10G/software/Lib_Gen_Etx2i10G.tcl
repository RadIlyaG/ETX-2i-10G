...
proc RetriveDutFam {{dutInitName ""}} {
  ...
  if {[string match *10G-B* $dutInitName]==1 || [string match *10G_C* $dutInitName]==1} {
    ...
    if {[string match *B.AC.* $dutInitName]==1 || \
        ...
        [string match *B_OFK.8.5.* $dutInitName]==1 || [string match *B_EFF.8.5.* $dutInitName]==1} {
      ...
      puts "if 011"
              ...
              [string match *B_GCN.19.*.4SFPP4SFP $dutInitName]==1} {
      puts "if 012"
...
#     }
#   }
  if {[string match *.PTP.* $dutInitName]==1 || [string match *.G.* $dutInitName]==1} {
    foreach {b r p d ps np up} [split $gaSet(dutFam) .] {
      set gaSet(dutFam) $b.$r.P.$d.$ps.$np.$up  
    }
  }
...
proc InformAboutNewFiles {} {
  ...
  if {[llength $::newFilesL]>0} {
    ...
    if {$res=="Yes"} {
      ...
      # 11:05 16/12/2025 append mess "\rfile://R:\\IlyaG\\$pathTail\r"
      # R:/IlyaG/$pathTail
...
proc _LY_wait {} {
  ...
  set LY_wait_list [list "ETX-2I-10G_LY.ACR.24SFP.tcl" "ETX-2I-10G_LY.ACR.2SFPP.24SFP.tcl" \
	...
	"ETX-2I-10G_FT.H.AC.4SFPP.12S12U.tcl"]
  ...
  if {$wait_option && \
    (([string match *LY* $gaSet(DutInitName)] && $gaSet(dbrSW)=="6.8.2(0.75)") || \
	 ...
	 ([string match *ETX-2I-10G_FT.H.AC.4SFPP.12S12U* $gaSet(DutInitName)] && $gaSet(dbrSW)=="6.8.5(1.81)") )} {
